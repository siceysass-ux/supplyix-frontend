...
const SettingsPage: React.FC<SettingsPageProps> = ({
    ...
    const [newAffiliateRate, setNewAffiliateRate] = useState('5');
    const [newValidPlans, setNewValidPlans] = useState<string[]>([]);
    ...
    const handlePlanDragStart = (e: React.DragEvent, index: number) => {
        setDraggedPlanIndex(index);
        e.dataTransfer.effectAllowed = 'move';
    };

    const handlePlanDrop = (e: React.DragEvent, droppedOnIndex: number) => {
        e.preventDefault();
        if (draggedPlanIndex === null || draggedPlanIndex === droppedOnIndex) {
            setDraggedPlanIndex(null);
            return;
        }
        const plans = [...localPlans];
        const [reorderedPlan] = plans.splice(draggedPlanIndex, 1);
        plans.splice(droppedOnIndex, 0, reorderedPlan);
        setLocalPlans(plans);
        setDraggedPlanIndex(null);
    };
    ...
    const handlePopupImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                // Show loading state
                setLocalPopup(prev => ({ ...prev, imageUrl: 'uploading...' }));

                // Upload to R2
                const { uploadImage } = await import('../../../src/services/api');
                const result = await uploadImage(file);

                // Update with permanent R2 URL
                setLocalPopup(prev => ({ ...prev, imageUrl: result.url }));
            } catch (error) {
                console.error('Image upload failed:', error);
                alert('Görsel yüklenemedi. Lütfen tekrar deneyin.');
                setLocalPopup(prev => ({ ...prev, imageUrl: '' }));
            }
        }
    };
    ...
    const handleAddCode = () => {
        ...
        const affiliateValue = parseFloat(newAffiliateRate);

        console.log('Adding code:', { codeValue, discountValue, affiliateValue });
        ...
        if (codeValue && !localCodes.some(c => c.code === codeValue)) {
            const newCodeObj: InfluencerCode = {
                ...
                affiliateRate: !isNaN(affiliateValue) ? affiliateValue : 5,
                usageCount: 0,
                totalEarnings: 0,
                validPlans: newValidPlans.length > 0 ? newValidPlans : undefined
            };
            console.log('New code object:', newCodeObj);
            const updatedCodes = [...localCodes, newCodeObj];
            setLocalCodes(updatedCodes);
            handleSave('codes', () => {
                console.log('Saving codes to API:', updatedCodes);
                onUpdateInfluencerCodes(updatedCodes);
            });
            setNewCode('');
            setNewDiscount('');
            setNewAffiliateRate('5');
            setNewValidPlans([]);
        } else {
            console.error('Code validation failed:', { codeValue, exists: localCodes.some(c => c.code === codeValue) });
        }
    };
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-4">
                    {localPlans.map((plan, index) => (
                        <div
                            key={plan.name}
                            draggable
                            onDragStart={(e) => handlePlanDragStart(e, index)}
                            onDragOver={handleDragOver}
                            onDrop={(e) => handlePlanDrop(e, index)}
                            className={`grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-3 rounded-lg border cursor-move transition-all ${draggedPlanIndex === index ? 'bg-primary/20 opacity-50 border-primary' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'
                                }`}
                        >
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-2 mb-4">
                    <input type="text" value={newCode} onChange={(e) => setNewCode(e.target.value)} placeholder="Referans kodu..." className="sm:col-span-5 bg-slate-50 p-2 rounded-md border border-slate-300" />
                    <input type="number" value={newDiscount} onChange={(e) => setNewDiscount(e.target.value)} placeholder="İndirim (%)" className="sm:col-span-2 bg-slate-50 p-2 rounded-md border border-slate-300" />
                    <input type="number" value={newAffiliateRate} onChange={(e) => setNewAffiliateRate(e.target.value)} placeholder="Komisyon (%)" className="sm:col-span-2 bg-slate-50 p-2 rounded-md border border-slate-300" />
                    <div className="sm:col-span-12 flex flex-wrap gap-2 mt-2">
                        <span className="text-sm font-semibold text-slate-600 self-center mr-2">Geçerli Paketler:</span>
                        {localPlans.map(plan => (
                            <label key={plan.name} className={`cursor-pointer px-3 py-1 rounded-full text-xs font-bold border transition-colors ${newValidPlans.includes(plan.name) ? 'bg-primary text-white border-primary' : 'bg-slate-100 text-slate-600 border-slate-300 hover:border-primary'}`}>
                                <input
                                    type="checkbox"
                                    className="hidden"
                                    checked={newValidPlans.includes(plan.name)}
                                    onChange={(e) => {
                                        if (e.target.checked) {
                                            setNewValidPlans(prev => [...prev, plan.name]);
                                        } else {
                                            setNewValidPlans(prev => prev.filter(p => p !== plan.name));
                                        }
                                    }}
                                />
                                {plan.name}
                        ...
                        {newValidPlans.length === 0 && <span className="text-xs text-slate-400 self-center italic">(Tümü için geçerli)</span>}
                    </div>
                    <button onClick={handleAddCode} className="sm:col-span-12 mt-2 bg-primary text-white font-semibold px-4 py-2 rounded-lg text-sm hover:bg-primary-focus w-full sm:w-auto ml-auto">Oluştur</button>
                </div>
                <div className="space-y-2">
                    {localCodes.map(code => (
                        <div key={code.id} className="flex items-center gap-2 bg-slate-50 p-3 rounded-md border border-slate-200">
                            <div className="flex-grow">
                                ...
                                <div className="flex items-center gap-4 text-xs text-slate-500">
                                    <span>👥 {code.usageCount || 0} kullanım</span>
                                    {typeof code.affiliateRate === 'number' && (
                                        <>
                                            <span>💰 Komisyon: %{code.affiliateRate}</span>
                                            <span>💵 Kazanç: ${(code.totalEarnings || 0).toFixed(2)}</span>
                                        </>
                                    )}
                                    {code.validPlans && code.validPlans.length > 0 && (
                                        <span className="ml-2 text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded">
                                            Paketler: {code.validPlans.join(', ')}
                                        </span>
                                    )}
                                </div>
                            </div>
