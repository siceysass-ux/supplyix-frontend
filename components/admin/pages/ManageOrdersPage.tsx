                                ...
                                <div className="flex items-center justify-between col-span-1 md:col-span-2">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-slate-800 dark:text-slate-100">{order.shippingCarrier} - <a href={`https://t.17track.net/en#nums=${order.trackingNumber}`} target="_blank" rel="noopener noreferrer" className="font-mono text-primary hover:underline">{order.trackingNumber}</a></p>
                                        </div>
                                    </div>
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-md w-full" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-slate-200 dark:border-slate-700">
                    <h2 className="text-xl font-bold text-dark-blue dark:text-slate-100">Kargo Takip Numarası Ekle</h2>
                ...
                <div className="p-6 space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            required
                        />
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Takip numarası girildiğinde sipariş durumu otomatik "Kargoda" olacak.</p>
                    </div>
                </div>
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3 rounded-b-xl">
...
const ManageOrdersPage: React.FC<ManageOrdersPageProps> = ({ orders, onUpdateOrderStatus, onUpdateTrackingInfo }) => {
    ...
    const handleExportToExcel = () => {
        const dataToExport = filteredOrders.map(order => {
            ...
            return {
                'Sipariş ID': order.id,
                'Müşteri': shippingAddress.consignee,
                'Durum': order.status,
                'Sipariş Tarihi': formatDateForExcel(order.creationDate),
                'Ürün Sayısı': products.length,
                'Ara Toplam': order.subtotal,
                'Kargo': order.shippingTotal,
                'Toplam': order.total,
                'Kargo Takip No': order.trackingNumber || '-'
            };
        });

        const filename = `siparisler_${new Date().toLocaleDateString('tr-TR').replace(/\./g, '-')}`;
        exportToExcel(dataToExport, filename, 'Siparişler');
    };
        ...
        <>
            ...
            {editingOrder && (
                <TrackingInfoModal
                    ...
                    onSave={(_carrier, trackingNo) => {
                        onUpdateTrackingInfo(editingOrder.id, trackingNo);
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    ...
                    <button
                        ...
                        className="font-semibold text-sm bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 inline-flex items-center gap-2"
                    >
                        ...
                        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                            {filteredOrders.map(order => (
                                <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                    ...
                                    <td className="px-6 py-4 font-medium text-dark-blue dark:text-slate-100">{typeof order.shippingAddress === 'string' ? JSON.parse(order.shippingAddress).consignee : order.shippingAddress.consignee}</td>
                                        ...
                                        {order.shippingCarrier && order.trackingNumber ? (
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <a href={`https://t.17track.net/en#nums=${order.trackingNumber}`} target="_blank" rel="noopener noreferrer" className="text-xs text-slate-500 dark:text-slate-400 hover:text-primary hover:underline">{order.trackingNumber}</a>
                                                </div>
